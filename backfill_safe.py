...
def calc_smartrr_sales_volume_from_orders(orders, now_str, brand_name, period_key, period_start, period_end):
    ...
    for (product, sku), vals in sorted(agg.items(), key=lambda kv: (-kv[1]["qty"], kv[0][0])):
        rows.append([
            ...
            product, sku, vals["qty"], vals["gross"],
            source, "Shopify order processed date fallback", "active subscription status not available in historical Shopify order fallback", examples.get((product, sku), "")
        ])
    return rows
...
def write_smartrr_product_rows(gc, sheet_id, smartrr_rows):
    ...
    try:
        ...
        ws = sh.add_worksheet("smartrr_product_volume", rows=max(100, len(smartrr_rows)+10), cols=len(headers))
    ws.clear()
    ...
    for brand, cfg in STORES.items():
        ...
        if brand == "cavali":
            ...
            for rr in smartrr_rows:
                totals[rr[5]] = totals.get(rr[5], 0) + int(float(rr[7] or 0))
