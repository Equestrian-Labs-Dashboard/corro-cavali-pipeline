...
def _clean_product_name(product):
    """Normalize dirty/blank product names before writing dashboard rows."""
    txt = str(product or "").strip()
    low = txt.lower()
    if (
        not txt
        or txt in ("ø", "?", "-", "--", "None", "null")
        or low.startswith("[sin nombre")
        or low.startswith("sin nombre")
        or "id=" in low and "sin nombre" in low
    ):
        return "Other"
    return txt[:90]
...
def _line_product_label(li):
    """Return a clean product/variant label from a Shopify order line item."""
    raw = str(
    ...
    # Canonical labels as seen in dashboard/Smartrr.
    ...
    if "signature box subscription" in t or "the signature box" in t or "signature box" in t:
        return "The Signature Box Subscription" if "subscription" in t else "The Signature Box"
    if "premier box subscription" in t or "the premier box" in t or "premier box" in t:
        return "The Premier Box Subscription" if "subscription" in t else "The Premier Box"
    ...
    if "sona" in t and "bundle" in t:
        return "Sona Bundle"
    if "kriste" in t or "training course" in t:
        return "Kriste Kehoe Training Course"
...
def _is_subscription_line_item(li, product_label=""):
    ...
    if not isinstance(li, dict):
        return False
    if li.get("selling_plan_allocation") or li.get("selling_plan_id") or li.get("selling_plan_name"):
        return True
...
def calc_smartrr_sales_volume_from_orders(orders, now_str, brand_name, period_key, period_start, period_end):
    """Build Cavali order-volume rows for smartrr_product_volume.

    HARD SEPARATION:
      - total_quantity / gross_revenue = Shopify order sales volume only.
      - new_subscribers = always 0 here. New subscribers are Smartrr-only and are written by the daily pipeline.
      - active/paused/cancelled = always 0 here. Current subscriber totals are Smartrr-only.
      - row_type=subscription_sales_volume for subscription/selling-plan order lines.
      - row_type=sales_volume_only for one-time order lines.
    """
    if brand_name != "cavali":
        return []
    ...
    for o in orders or []:
        for li in o.get("line_items", []) or []:
            ...
            sku = str(li.get("sku") or li.get("variant_id") or li.get("product_id") or "ø").strip() or "ø"
            is_sub = bool(_is_subscription_line_item(li, product))
            key = (product, sku, is_sub)
            rec = agg.setdefault(key, {"qty": 0, "gross": 0.0, "ids": []})
            rec["qty"] += qty
            rec["gross"] = round(rec["gross"] + _line_gross_revenue(li), 2)
    ...
    for (product, sku, is_sub), vals in sorted(agg.items(), key=lambda kv: (-kv[1]["qty"], kv[0][0].lower(), kv[0][2])):
        qty = int(vals["qty"])
        source = "Shopify order line_items — Subscription Sales Volume only" if is_sub else "Shopify order line_items — One-Time Sales Volume only"
        active_filter = "row_type=subscription_sales_volume" if is_sub else "row_type=sales_volume_only"
        rows.append([
            now_str, brand_name, period_key, str(period_start), str(period_end),
            _clean_product_name(product), sku,
            qty, 0,
            0, 0, 0,
            round(vals["gross"], 2),
            source,
...
def write_smartrr_product_rows(gc, sheet_id, smartrr_rows):
    ...
    cleaned = []
    for r in smartrr_rows:
        row = list(r)
        while len(row) < len(SMARTRR_PRODUCT_VOLUME_HEADERS):
            row.append("")
        row[5] = _clean_product_name(row[5])
        # Do not fabricate active subscribers from sales quantity.
        if row[9] in (None, "", "ø", "None", "null"):
            row[9] = 0
        if row[10] in (None, "", "ø", "None", "null"):
            row[10] = 0
        if row[11] in (None, "", "ø", "None", "null"):
            row[11] = 0
        cleaned.append(row[:len(SMARTRR_PRODUCT_VOLUME_HEADERS)])

    ws.clear()
    ws.append_row(SMARTRR_PRODUCT_VOLUME_HEADERS)
    if cleaned:
        ws.append_rows(cleaned, value_input_option="USER_ENTERED")
...
def main():
    ...
    for brand, cfg in STORES.items():
        ...
        if brand == "cavali" and smartrr_rows:
            ...
            # The previous version accidentally read rr[6] (SKU) as a number,
            # which failed on SKUs like FALL403. Use total_quantity at rr[7].
            totals = {}
            for rr in smartrr_rows:
                try:
                    product_name = str(rr[5] if len(rr) > 5 else "Other")
                    qty = int(float(rr[7] if len(rr) > 7 and rr[7] not in (None, "") else 0))
                except Exception:
                    product_name = str(rr[5] if len(rr) > 5 else "Other")
                    qty = 0
        ...
        # Smartrr: snapshot actual de suscriptores activos por tipo de caja.
        # No es histórico por periodo; se escribe como referencia del estado actual.
        smartrr_row = fetch_smartrr_active_subs(brand, url, token)
        write_smartrr(gc, sid, smartrr_row)

        print(f"\n  ✓ {brand.upper()} completado: "
              f"{len(kpi_rows)} KPI + {len(rs_rows)} RS + {len(nvr_rows)} NVR + {len(smartrr_rows)} Smartrr filas")
