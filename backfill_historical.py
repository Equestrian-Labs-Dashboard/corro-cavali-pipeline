...
def calc_smartrr_sales_volume_from_orders(orders, now_str, brand_name, period_key, period_start, period_end):
    """Build Cavali smartrr_product_volume rows with the same schema used by dashboard v47.

    Backfill fallback source: Shopify order line_items for the selected period.
    Important: active_subscribers_current is written as active_min = new quantity,
    so historical/backfill rows never render blank active boxes in the dashboard.
    The daily pipeline can later enrich these same rows with real Smartrr ACTIVE/PAUSED totals.
    """
    if brand_name != "cavali":
        return []

    source = "Shopify order line_items fallback for Smartrr product volume"
    date_basis = "Shopify order processed date fallback"
    active_filter = "Fallback minimum total: active/paused Smartrr product total unavailable"
    agg = {}
    ...
    if not agg:
        rows.append([
            ...
            source,
            date_basis,
            "No product line items found for this period",
            "",
        ])
        return rows
...
def write_smartrr_product_rows(gc, sheet_id, smartrr_rows):
    ...
    for r in smartrr_rows:
        ...
        if row[9] in (None, "", "ø", "None", "null"):
            row[9] = row[8] if row[8] not in (None, "", "ø", "None", "null") else row[7]
