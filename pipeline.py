...
SMARTRR_PRODUCT_VOLUME_HEADERS = [
    ...
    # Global snapshot totals by product up to the selected period end date.
    # Named *_current so the dashboard JS reads them directly via activeOf / pausedOf.
...
def build_smartrr_product_volume_rows(now_str, brand_name, active_states, period_defs):
    ...
    # Diagnostic: dump the first purchase state to understand its structure
    if active_states:
        first = active_states[0]
        print(f"    smartrr debug: first state keys = {list(first.keys())[:30]}")
        # Show top-level fields that might hold product/created info
        for dbg_key in ("purchasableTitle", "purchasable_title", "planTitle", "plan_title",
                        "productTitle", "product_title", "variantTitle", "variant_title",
                        ...
                        "shopifyId", "shopify_id", "externalId", "external_id",
                        "quantity", "qty"):
            v = first.get(dbg_key)
            if v not in (None, ""):
                print(f"      state.{dbg_key} = {str(v)[:120]}")
        # Show nested list-type fields (these might be line items)
        for dbg_key, dbg_val in first.items():
            if isinstance(dbg_val, (list, dict)) and dbg_key not in ("_smartrr_status_hint",):
                print(f"      state.{dbg_key} (type={type(dbg_val).__name__}, len={len(dbg_val) if hasattr(dbg_val,'__len__') else '?'}): {str(dbg_val)[:200]}")
    ...
    for sub in active_states:
        ...
        # --- FALLBACK: if Smartrr doesn't nest line items, treat the purchase state itself as the line ---
        # This happens when the API returns product info at the top level of each purchase state.
        if not candidate_lines:
            # Try to extract product name directly from the purchase state
            prod = ""
            for fk in (
                "purchasableAndPurchasableVariantName", "purchasable_and_purchasable_variant_name",
                ...
                if v and str(v).strip() not in ("", "ø", "Default Title"):
                    ...
                    break
            if not prod:
                prod = _line_product(sub)  # deep search fallback

            sku = _line_sku(sub)

            # Created date: use the state's own createdAt
            created = None
            for ck in ("createdAt", "created_at", "createdDate", "created_date", "created"):
                v = sub.get(ck)
                if v:
                    ...
                    if created:
                        break
            if not created:
                created = _line_created_date(sub)

            if not created or not prod:
                skipped_no_created += 1
                continue

            qty = 1
            for qk in ("quantity", "qty", "subscriptionQuantity", "subscription_quantity"):
                v = sub.get(qk)
                if v not in (None, ""):
                    try:
                        qty = max(1, int(float(str(v))))
                        break
                    except Exception:
                        pass

            gross = _line_revenue(sub, qty)
            lid = _line_id(sub) or f"{state_group}|{prod}|{sku}|{created}|{qty}"
