...
def build_smartrr_product_volume_rows(now_str, brand_name, active_states, period_defs):
    ...
    if active_states:
        ...
        for dbk in ("stLineItems", "lineItems", "orderLineItems", "items"):
            ...
            if isinstance(v, list) and v:
                ...
                inner = v[0]
                for dbk2 in ("purchasable", "variant", "product", "purchasableVariant", "vnt"):
                    vv = inner.get(dbk2)
                    if vv is not None:
                        if isinstance(vv, dict):
                            print(f"    smartrr debug: {dbk}[0].{dbk2} keys = {list(vv.keys())}")
                            # Print full vnt contents so we can see name/title fields
                            print(f"    smartrr debug: {dbk}[0].{dbk2} values = {str(dict(list(vv.items())[:15]))[:400]}")
                        else:
                            print(f"    smartrr debug: {dbk}[0].{dbk2} = {str(vv)[:200]}")
    ...
    def _extract_product_name_from_st_line(st_line):
        ...
        # Nested: purchasable / purchasableVariant / variant / vnt (Smartrr compact key)
        for nested_key in ("purchasable", "purchasableVariant", "variant", "product", "vnt"):
            ...
            for fk in ("name", "title", "productTitle", "product_title", "variantTitle", "variant_title",
                        "purchasableAndPurchasableVariantName", "displayName", "display_name"):
                v = nested.get(fk)
                if v and str(v).strip() not in ("", "ø", "Default Title"):
                    return str(v).strip()
            # vnt may nest a purchasable/product object
            for sub_key in ("purchasable", "product", "purchasableVariant"):
                sub_obj = nested.get(sub_key)
                if isinstance(sub_obj, dict):
                    for fk in ("name", "title", "productTitle", "product_title"):
                        v = sub_obj.get(fk)
                        if v and str(v).strip() not in ("", "ø", "Default Title"):
                            return str(v).strip()

        # Last resort: currentSku as product label
        sku_val = st_line.get("currentSku") or st_line.get("sku") or st_line.get("SKU")
        if sku_val and str(sku_val).strip():
            return str(sku_val).strip()

        return ""

    def _extract_sku_from_st_line(st_line):
        for fk in ("sku", "SKU", "currentSku", "current_sku"):
            v = st_line.get(fk)
    ...
    def _extract_price_from_st_line(st_line, qty):
        for fk in ("price", "basePrice", "base_price", "linePrice", "line_price", "totalPrice", "total_price",
        ...
        for nested_key in ("purchasable", "purchasableVariant", "variant", "vnt"):
            nested = st_line.get(nested_key)
            if isinstance(nested, dict):
                for fk in ("price", "basePrice", "base_price", "unitPrice", "unit_price"):
                    v = nested.get(fk)
                    if v not in (None, ""):
                        return _money_to_usd(v) * qty
        return 0.0
    ...
    for sub in active_states:
        ...
        for st_line in (st_lines if isinstance(st_lines, list) else []):
            ...
            # One-time debug: show first non-deleted stLineItem full contents
            if not _st_debug_done and active_states.index(sub) == 0:
                _st_debug_done = True
                print(f"    smartrr stLine keys={list(st_line.keys())}")
                for dk in ("vnt", "purchasable", "variant", "product", "purchasableVariant"):
                    dv = st_line.get(dk)
                    if dv is not None:
                        print(f"    smartrr stLine.{dk} = {str(dv)[:400]}")
                print(f"    smartrr stLine.currentSku = {st_line.get('currentSku')} sku={st_line.get('sku')}")
                print(f"    smartrr stLine.createdDate = {st_line.get('createdDate')} createdAt={st_line.get('createdAt')}")
