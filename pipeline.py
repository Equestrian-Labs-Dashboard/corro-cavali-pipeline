    ...
    for pk, ps, pe, ds, de in period_ranges:
        ...
        for prod, sku in sorted(
            ...
            rows.append([
                ...
                round(gross, 2),
                "Smartrr purchase-state ACTIVE/PAUSED/CANCELLED totals + Subscription Sales Volume",
                "State createdAt / Order Line Item Created Date",
                "purchaseStateStatus=ACTIVE/PAUSED/CANCELLED;row_type=smartrr_subscription_sales_volume",
                "; ".join(ids),
            ])
...
def build_smartrr_product_volume_from_orders(now_str, brand_name, period, start, end, orders):
    """
    Cavali product-volume fallback from Shopify order line_items.

    IMPORTANT separation:
      - total_quantity / gross_revenue = Sales Volume by Product-Variant for the selected range.
      - subscription line items are written as Subscription Sales Volume only.
      - new_subscribers stays 0 in Shopify fallback rows; real new subscribers come from Smartrr purchase-state rows only.
      - normal one-off products stay in the sales-volume table and DO NOT inflate subscribers.
    """
    if brand_name != "cavali":
        return []
    ...
    for o in orders or []:
        for li in (o.get("line_items") or []):
            ...
            key = (product, sku)
            rec = buckets.setdefault(key, {"qty": 0, "subscription_qty": 0, "gross": 0.0, "ids": []})
            rec["qty"] += qty
            if _is_subscription_line_item(li, product):
                rec["subscription_qty"] += qty
    ...
    for (product, sku), v in sorted(buckets.items(), key=lambda kv: (-kv[1]["qty"], kv[0][0].lower())):
        qty = int(v["qty"])
        sub_qty = int(v.get("subscription_qty", 0) or 0)
        is_sub = sub_qty > 0
        rows.append([
            ...
            0, 0, 0,
            round(v["gross"], 2),
            "Shopify order line_items fallback for Subscription Sales Volume only; row_type=subscription_sales_volume" if is_sub else "Shopify order line_items fallback for One-Time Sales Volume only; row_type=sales_volume_only",
    ...
    if rows:
        ...
        for r in rows[:6]:
            print(f"      fallback product row: {r[5]} · sku={r[6]} · sales_qty={r[7]} · new_subscribers={r[8]} · active_min={r[9]} · gross={r[12]} · {r[15]}")
    return rows
