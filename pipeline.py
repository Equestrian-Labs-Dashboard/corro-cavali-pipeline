...
SMARTRR_PRODUCT_VOLUME_HEADERS = [
    ...
    "total_quantity", "new_subscribers", "active_subscribers_to_date", "gross_revenue",
...
def fetch_smartrr_active_purchase_states(brand_name):
    """Fetch ACTIVE purchase states once. No Shopify subscription-contract lookup."""
...
def build_smartrr_product_volume_rows(now_str, brand_name, active_states, period_defs):
    ...
    For every selected period/product row:
      - total_quantity / new_subscribers = active line-items created inside that selected period.
    ...
    This lets the dashboard show, per product:
      Big number  = Total Subscribers to date.
      Small number = New Subscribers in the selected filter.
    """
    ...
    for pk, ps, pe, ds, de in period_ranges:
        # Use all currently ACTIVE subscription line-items up to the selected end date.
        total_buckets = {}
        new_buckets = {}
        ...
        for item in line_items:
            if item["created"] <= de:
                ...
                rec = total_buckets.setdefault(key, {"total": 0, "ids": []})
                rec["total"] += item["qty"]
                if len(rec["ids"]) < 5:
        ...
        # Include every active product with total_to_date > 0, even if new_subscribers is 0 in this period.
        all_keys = set(total_buckets.keys()) | set(new_buckets.keys())
        for prod, sku in sorted(all_keys, key=lambda k: (-total_buckets.get(k, {}).get("total", 0), k[0])):
            total_to_date = total_buckets.get((prod, sku), {}).get("total", 0)
            new_count = new_buckets.get((prod, sku), {}).get("new", 0)
            gross = new_buckets.get((prod, sku), {}).get("gross", 0.0)
            ids = new_buckets.get((prod, sku), {}).get("ids") or total_buckets.get((prod, sku), {}).get("ids") or []
            if total_to_date <= 0 and new_count <= 0:
                continue
            rows.append([
                now_str, brand_name, pk, ps, pe,
                prod, sku,
                new_count, new_count, total_to_date, round(gross, 2),
    ...
    print(f"    smartrr_product_volume: active_line_items={len(line_items)} rows={len(rows)} skipped_no_created_date={skipped_no_created}")
    tests = [r for r in rows if r[2] in ("2026-04", "mtd_2026-05")][:10]
    for r in tests[:8]:
        print(
            f"      smartrr product test: {r[5]} · sku={r[6]} · "
            f"new={r[8]} · total_to_date={r[9]} · gross={r[10]} · period={r[2]}"
        )
    return rows
...
def build_smartrr_product_volume_from_orders(now_str, brand_name, period, start, end, orders):
    ...
    for (product, sku), v in sorted(buckets.items(), key=lambda kv: (-kv[1]["qty"], kv[0][0].lower())):
        ...
        rows.append([
            ...
            product, sku,
            int(v["qty"]), int(v["qty"]), int(v["qty"]), round(v["gross"], 2),
            "Shopify order line_items fallback for Smartrr product volume",
            "Shopify order processed date fallback",
            "Fallback minimum total: active Smartrr product total unavailable",
            "; ".join(v["ids"]),
        ])
    ...
    if rows:
        ...
        for r in rows[:6]:
            print(f"      fallback product row: {r[5]} · sku={r[6]} · new={r[8]} · gross={r[10]}")
    return rows
...
def merge_smartrr_product_volume_rows(order_rows, active_rows):
    ...
    total-to-date when Smartrr exposes usable product line data.
    ...
    for r in active_rows:
        try:
            exact_key = (str(r[2]), str(r[5]), str(r[6]))
            active_exact[exact_key] = r
            loose_key = (str(r[2]), _product_match_key(r[5]))
            # Keep the row with the largest total_to_date for the product.
            prev = active_product.get(loose_key)
            if not prev or _to_number(r[9], 0) > _to_number(prev[9], 0):
    ...
    for r in order_rows:
        try:
            key = (str(r[2]), str(r[5]), str(r[6]))
            loose_key = (str(r[2]), _product_match_key(r[5]))
            a = active_exact.get(key) or active_product.get(loose_key)
            row = list(r)

            if a:
                used_active.add((str(a[2]), str(a[5]), str(a[6])))
                # Keep exact period new/gross from Shopify/Smartrr period rows,
                # but use ACTIVE total-to-date from Smartrr when available.
                if len(a) > 9 and not _is_blank_number(a[9]):
                    row[9] = a[9]
                if len(a) > 13 and a[13] not in (None, ""):
                    row[13] = a[13]
                if len(a) > 14 and a[14] not in (None, ""):
                    row[14] = a[14]
                row[11] = "Smartrr ACTIVE total + period product rows"
                row[12] = "Order Line Item Created Date / Shopify processed date fallback"

            # Never leave active_subscribers_to_date blank. Minimum safe value is period new count.
            if len(row) > 9 and _is_blank_number(row[9]):
                row[9] = row[8] if len(row) > 8 else row[7]
            merged.append(row)
        except Exception:
            merged.append(r)

    # Add active products with 0 new subscribers in this period, so every active product can still show.
    for r in active_rows:
        try:
            active_key = (str(r[2]), str(r[5]), str(r[6]))
            if active_key not in used_active:
                row = list(r)
                if len(row) > 8 and _is_blank_number(row[8]):
                    row[8] = 0
                if len(row) > 7 and _is_blank_number(row[7]):
                    row[7] = row[8]
                if len(row) > 9 and _is_blank_number(row[9]):
                    row[9] = row[8] if len(row) > 8 else 0
                merged.append(row)
        except Exception:
            merged.append(r)

    if active_rows:
        print(f"    smartrr_product_volume merge: {len(order_rows)} order rows + {len(active_rows)} active rows => {len(merged)} rows")
    else:
        print(f"    smartrr_product_volume merge: active rows unavailable; using {len(order_rows)} order fallback rows")
    return merged
...
def write_smartrr_product_volume(gc, sheet_id, rows, periods_to_replace):
    ...
    for r in rows or []:
        ...
        while len(row) < len(SMARTRR_PRODUCT_VOLUME_HEADERS):
            row.append("")
        # Minimum safeguard: product card total cannot be empty.
