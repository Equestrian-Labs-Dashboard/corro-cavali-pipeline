...
# ─────────────────────────────────────────────────────────────────
# CAVALI PRODUCT VOLUME BY PRODUCT-VARIANT
# ─────────────────────────────────────────────────────────────────
def _clean_product_variant(li):
    """Return the product name as Smartrr/Shopify displays it, without bucketing to Other."""
    for k in ("title", "name", "product_title", "variant_title"):
        v = li.get(k)
        if v not in (None, ""):
            txt = str(v).strip()
            # Shopify sometimes returns "Product - Variant" in name. Keep it readable but not empty.
            return txt or "ø"
    return "ø"


def _line_sku(li):
    return str(li.get("sku") or li.get("variant_id") or "ø").strip() or "ø"


def _line_revenue(li):
    """Gross line revenue before order-level allocation; matches Smartrr sales-volume style more closely than net order subtotal."""
    qty = int(float(li.get("quantity") or 0))
    for k in ("price", "pre_tax_price", "discounted_price"):
        try:
            v = li.get(k)
            if v not in (None, ""):
                return round(float(v) * qty, 2)
        except Exception:
            pass
    try:
        return round(float(li.get("total_discount", 0) or 0), 2)
    except Exception:
        return 0.0


def calc_smartrr_product_volume_rows(orders, now_str, brand_name, period, start, end):
    """
    Period-specific product volume for Cavali section 06.
    IMPORTANT: this is by selected period. No fallback to latest/lifetime totals.
    Uses order line_items so product names/SKUs stay exact and nothing is mapped into Other.
    """
    ...
    for o in orders or []:
        for li in o.get("line_items", []) or []:
            ...
            if qty <= 0:
                continue
            product = _clean_product_variant(li)
            sku = _line_sku(li)
            key = (product, sku)
            if key not in buckets:
                buckets[key] = {"qty": 0, "revenue": 0.0}
            buckets[key]["qty"] += qty
            buckets[key]["revenue"] += _line_revenue(li)

    rows = []
    for (product, sku), vals in sorted(buckets.items(), key=lambda kv: (-kv[1]["qty"], kv[0][0].lower())):
        rows.append([
            now_str, brand_name, period, str(start), str(end),
            product, sku, int(vals["qty"]), round(vals["revenue"], 2),
            "Smartrr period mirror · Shopify order line_items · all products in selected range",
            "",
        ])
    if brand_name == "cavali":
        print(f"    smartrr sales volume rows for {period}: {len(rows)}")
        for r in rows[:8]:
            print(f"      smartrr sales volume test: {r[5]} · sku={r[6]} · qty={r[7]} · gross={r[8]:,.2f}")
    return rows
...
# ─────────────────────────────────────────────────────────────────
def write_all(gc, sheet_id, kpi_rows, rs_rows, nvr_rows, brand_name, smartrr_volume_rows=None):
    sh = gc.open_by_key(sheet_id)
    ...
    # Period-specific product volume for Cavali. This replaces the old active-subscriber snapshot for section 06.
    if brand_name == "cavali":
        try:    ws_sv = sh.worksheet("smartrr_product_volume")
        except: ws_sv = sh.add_worksheet("smartrr_product_volume", rows=600, cols=len(SMARTRR_VOLUME_HEADERS))

        sv_vals = ws_sv.get_all_values()
        existing_sv = {}
        if len(sv_vals) >= 2:
            ex_h = sv_vals[0]
            for r in sv_vals[1:]:
                m = _row_to_map(ex_h, r)
                key = (
                    str(m.get("period", "")).strip(),
                    str(m.get("product_variant", "")).strip(),
                    str(m.get("sku", "")).strip(),
                )
                if key[0] and key[1]:
                    existing_sv[key] = _map_to_row(SMARTRR_VOLUME_HEADERS, m)
        # Replace only periods produced in this run, and keep older historical periods.
        periods_now = {str(r[2]).strip() for r in (smartrr_volume_rows or [])}
        if periods_now:
            existing_sv = {k:v for k,v in existing_sv.items() if k[0] not in periods_now}
        for r in (smartrr_volume_rows or []):
            existing_sv[(str(r[2]).strip(), str(r[5]).strip(), str(r[6]).strip())] = r

        merged_sv = sorted(existing_sv.values(), key=lambda r: (_safe_date(r[3]), str(r[2]), -float(r[7] or 0), str(r[5])))
        ws_sv.clear()
        ws_sv.append_row(SMARTRR_VOLUME_HEADERS)
        if merged_sv:
            ws_sv.append_rows(merged_sv, value_input_option="USER_ENTERED")
        print(f"    smartrr_product_volume: {len(merged_sv)} rows")
...
def main():
    ...
    for brand_name, cfg in STORES.items():
        ...
        kpi_rows, rs_rows, nvr_rows, smartrr_volume_rows = [], [], [], []
        ...
        for it in periods_to_run:
            ...
            if brand_name == "cavali":
                smartrr_volume_rows.extend(calc_smartrr_product_volume_rows(ords, now_str, brand_name, pk, s, e))
        ...
        write_all(gc, cfg["sheet_id"], kpi_rows, rs_rows, nvr_rows, brand_name, smartrr_volume_rows)
