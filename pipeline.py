"""
Pipeline CORRO / CAVALI v4.5 — Smartrr totals safe
=============================================
...
- Escribe por producto:
  new_subscribers = nuevos del rango seleccionado.
  total_subscribers_to_date = activos + pausados acumulados hasta el fin del filtro.
  active_subscribers_to_date = activos acumulados hasta el fin del filtro.
  paused_subscribers_to_date = pausados acumulados hasta el fin del filtro.
- Si Smartrr no entrega totales por producto, NO copia nuevos como total; deja totales vacíos para no falsear datos.
...
SMARTRR_PRODUCT_VOLUME_HEADERS = [
    ...
    "product_variant", "sku",
    # total_quantity is kept for backward compatibility and equals new_subscribers for the selected period.
    "total_quantity", "new_subscribers",
    # Snapshot totals by product up to the selected period end date.
    "active_subscribers_to_date", "paused_subscribers_to_date", "total_subscribers_to_date",
...
def _canonical_product_name(v):
    """Normalize Cavali product names so Shopify and Smartrr rows merge by product.

    This is intentionally product-level, not SKU-level: the dashboard needs one card
    per product showing New Subscribers + Active Total + Paused Total.
    """
    raw = _norm_txt(v)
    if not raw or raw in ("ø", "Default Title", "Unknown Product"):
        return "Unknown Product"
    t = raw.lower()

    # These are Smartrr plan/group names, not products. Do not map them to totals.
    if ("quarterly membership" in t or "yearly membership" in t or "annual membership" in t) and \
       "cavali club membership" not in t and "junior" not in t:
        return "Unknown Product"
    ...
    if "signature" in t and "spring" in t:
        return "The Signature Spring Box"
    if "signature" in t:
        return "The Signature Box"
    if "cavali club membership" in t or "seasonal" in t:
        return "Cavali Club Membership"
    if "welcome box" in t:
        return "Welcome Box"
    if "fly spray" in t or "bugg" in t or "bug'n" in t or "buggn" in t:
        return "Stop Bugg’n Fly Spray 16oz"
    if "backpack" in t:
        return "Tryon Eco-Friendly Equestrian Backpack"

    # Clean common suffixes without changing unknown products too aggressively.
    raw = re.sub(r"\s+subscription$", "", raw, flags=re.I).strip()
    raw = re.sub(r"\s+default title$", "", raw, flags=re.I).strip()
    return raw or "Unknown Product"
...
def _line_product(line):
    ...
    for v in vals:
        ...
        if txt and txt not in ("ø", "Default Title"):
            return _canonical_product_name(txt)
    return "Unknown Product"
...
def _subscription_created_date(sub):
    """Best available subscription/purchase-state created date.

    Used only for active/paused totals-to-date. If Smartrr does not expose a
    date, we treat it as very old so current active/paused totals do not vanish.
    """
    val = (
        _dig(sub, "createdDate") or _dig(sub, "created_date") or
        _dig(sub, "createdAt") or _dig(sub, "created_at") or
        _dig(sub, "purchaseStateCreatedDate") or _dig(sub, "purchase_state_created_date") or
        _dig(sub, "purchaseStateCreatedAt") or _dig(sub, "purchase_state_created_at") or
        _dig(sub, "activatedAt") or _dig(sub, "activated_at") or
        _dig(sub, "startedAt") or _dig(sub, "started_at") or
        _first_deep(sub, [
            "createdDate", "created_date", "createdAt", "created_at",
            "purchaseStateCreatedDate", "purchaseStateCreatedAt",
            "activatedAt", "activated_at", "startedAt", "started_at",
        ])
    )
    parsed = _parse_smartrr_date(val)
    return parsed or date(1900, 1, 1)


def _subscription_product(sub):
    """Product/variant title directly from a Smartrr purchase-state.

    This fallback is important because some Smartrr payloads expose the product
    on the purchase-state but not as a nested order-line with Created Date.
    """
    vals = [
        _dig(sub, "product.title"), _dig(sub, "product.name"),
        _dig(sub, "productTitle"), _dig(sub, "product_title"),
        _dig(sub, "productName"), _dig(sub, "product_name"),
        _dig(sub, "variant.title"), _dig(sub, "variant.name"),
        _dig(sub, "variantTitle"), _dig(sub, "variant_title"),
        _dig(sub, "purchasable_and_purchasable_variant_name"),
        _dig(sub, "purchasableAndPurchasableVariantName"),
        _dig(sub, "items.0.productTitle"), _dig(sub, "items.0.product_title"),
        _dig(sub, "items.0.title"), _dig(sub, "items.0.name"),
        _dig(sub, "lineItems.0.productTitle"), _dig(sub, "lineItems.0.product_title"),
        _dig(sub, "lineItems.0.title"), _dig(sub, "lineItems.0.name"),
        _dig(sub, "stLineItems.0.productTitle"), _dig(sub, "stLineItems.0.product_title"),
        _dig(sub, "stLineItems.0.title"), _dig(sub, "stLineItems.0.name"),
        _first_deep(sub, [
            "purchasable_and_purchasable_variant_name", "purchasableAndPurchasableVariantName",
            "productVariantName", "product_variant_name", "productTitle", "product_title",
            "productName", "product_name", "variantTitle", "variant_title", "title", "name",
    ...
    # Avoid low-quality plan/group names unless nothing else exists.
    plan_fallback = (
        _dig(sub, "sellingPlan.name") or _dig(sub, "selling_plan.name") or
        _dig(sub, "sellingPlanName") or _dig(sub, "selling_plan_name") or
        _dig(sub, "subscriptionProgram.name") or _dig(sub, "subscription_program.name") or
        _dig(sub, "planName") or _dig(sub, "plan_name") or
        _dig(sub, "name") or _dig(sub, "title")
    )
    for v in vals:
        txt = _norm_txt(v)
        if txt and txt not in ("ø", "Default Title"):
            return _canonical_product_name(txt)
    txt = _norm_txt(plan_fallback)
    return _canonical_product_name(txt) if txt else "Unknown Product"


def _subscription_sku(sub):
    vals = [
        _dig(sub, "sku"), _dig(sub, "SKU"), _dig(sub, "variant.sku"),
        _dig(sub, "purchasableVariant.sku"), _dig(sub, "purchasable_variant.sku"),
        _dig(sub, "items.0.sku"), _dig(sub, "lineItems.0.sku"), _dig(sub, "stLineItems.0.sku"),
        _first_deep(sub, ["sku", "SKU", "purchasableVariantSku", "purchasable_variant_sku"]),
    ]
    for v in vals:
        txt = _norm_txt(v)
        if txt:
...
def _subscription_id(sub):
    vals = [
        _dig(sub, "id"), _dig(sub, "purchaseStateId"), _dig(sub, "purchase_state_id"),
        _dig(sub, "shopifyId"), _dig(sub, "shopify_id"),
        _dig(sub, "subscriptionId"), _dig(sub, "subscription_id"),
        _dig(sub, "externalSubscriptionId"), _dig(sub, "external_subscription_id"),
    ]
    for v in vals:
        txt = _norm_txt(v)
        if txt:
...
def _subscription_quantity(sub):
    q = (
        _dig(sub, "quantity") or _dig(sub, "qty") or
        _dig(sub, "items.0.quantity") or _dig(sub, "lineItems.0.quantity") or
        _dig(sub, "stLineItems.0.quantity") or _first_deep(sub, ["quantity", "qty"])
    )
    n = _to_number(q, 1.0)
    return int(n) if n and n > 0 else 1
...
def build_smartrr_product_volume_rows(now_str, brand_name, active_states, period_defs):
    ...
    For every selected period/product row:
      ...
      - paused_subscribers_to_date = PAUSED line-items for that product created from the beginning
        through the selected period end date.
      - total_subscribers_to_date = active + paused up to the selected period end date.

    This lets the dashboard show, per product:
      - New Subscribers in the selected filter.
      - Total Subscribers up to the filter end date.
    ...
    for sub in active_states:
        ...
        sub_created = _subscription_created_date(sub)
        candidate_lines = _candidate_line_dicts(sub)
        added_for_sub = 0

        for line in candidate_lines:
            if _line_deleted(line):
                continue
            created = _line_created_date(line) or sub_created
            if not created:
            ...
            if prod == "Unknown Product":
                continue
            sku = _line_sku(line)
            line_qty = _line_quantity(line)
            qty = 1  # subscriber count: one purchase-state line = one active/paused subscriber
            gross = _line_revenue(line, line_qty)
            lid = _line_id(line) or f"{state_group}|{_subscription_id(sub)}|{prod}|{sku}|{created}|{qty}|{gross}"
        ...
        # Critical fallback: some Smartrr purchase-state rows have the product on the
        # subscription itself but do not expose usable order-line dicts. We still need
        # those for ACTIVE/PAUSED totals, otherwise the dashboard shows New as Active.
        if added_for_sub == 0:
            prod = _subscription_product(sub)
            if prod == "Unknown Product":
                skipped_no_created += 1
                continue
            sku = _subscription_sku(sub)
            qty = 1  # subscriber count: one purchase state = one subscriber
            created = sub_created
            gross = 0.0
            sid = _subscription_id(sub)
            dedupe = f"{state_group}|subscription|{sid}|{prod}|{sku}"
            if dedupe not in seen_lines:
                seen_lines.add(dedupe)
                line_items.append({
                    "created": created,
                    "product": prod,
    ...
    for pk, ps, pe, ds, de in period_ranges:
        ...
        for item in line_items:
            key = (item["product"], "product_total")
        ...
        for prod, sku in sorted(
            ...
            rows.append([
                ...
                prod, sku,
                new_count, new_count,
                active_to_date, paused_to_date, active_to_date + paused_to_date,
                round(gross, 2),
    ...
    for r in tests[:8]:
        print(
            ...
            f"new={r[8]} · active_to_date={r[9]} · paused_to_date={r[10]} · total_to_date={r[11]} · gross={r[12]} · period={r[2]}"
        )
    return rows
...
def _shopify_line_product(li):
    """Return canonical Cavali product label from a Shopify order line item."""
...
def build_smartrr_product_volume_from_orders(now_str, brand_name, period, start, end, orders):
    ...
    Shopify order line_items are only reliable for NEW subscribers in the selected period.
    They are NOT a source for lifetime ACTIVE/PAUSED subscriber totals.
    Therefore active_subscribers_to_date and paused_subscribers_to_date are intentionally
    left blank in this fallback, so the dashboard never shows fake totals.
    """
    if brand_name != "cavali":
        return []
    ...
    for (product, sku), v in sorted(buckets.items(), key=lambda kv: (-kv[1]["qty"], kv[0][0].lower())):
        # Shopify order rows are ONLY the selected-period new subscribers.
        # Do NOT copy this value into active_subscribers_to_date; that was the bug
        # that made Active Total equal New Subscribers in the dashboard.
        rows.append([
            ...
            "", "", "",
            round(v["gross"], 2),
            "Shopify order line_items period rows; Smartrr totals required for active/paused",
            "Shopify order processed date fallback for period-new count",
            "New subscribers only; active/paused totals are not inferred from orders",
            "; ".join(v["ids"]),
        ])
    ...
    if rows:
        ...
        for r in rows[:6]:
            print(f"      fallback product row: {r[5]} · sku={r[6]} · new={r[8]} · active_total={r[9] or 'unknown'} · paused_total={r[10] or 'unknown'} · total_to_date={r[11] or 'unknown'} · gross={r[12]}")
    return rows
...
def merge_smartrr_product_volume_rows(order_rows, active_rows):
    ...
    for r in order_rows:
        try:
            ...
            if a:
                ...
                # Use Smartrr as the source of truth for the selected-period new subscribers
                # and for active/paused totals. Shopify order rows are only fallback when
                # Smartrr product rows are unavailable.
                if len(a) > IDX_NEW and not _is_blank_number(a[IDX_NEW]):
                    row[IDX_NEW] = a[IDX_NEW]
                    row[IDX_TOTAL_QTY] = a[IDX_NEW]
                if len(a) > IDX_GROSS and not _is_blank_number(a[IDX_GROSS]):
                    row[IDX_GROSS] = a[IDX_GROSS]
                ...
                if len(a) > IDX_TOTAL_TO_DATE and not _is_blank_number(a[IDX_TOTAL_TO_DATE]):
                    row[IDX_TOTAL_TO_DATE] = a[IDX_TOTAL_TO_DATE]
                elif not _is_blank_number(row[IDX_ACTIVE]) or not _is_blank_number(row[IDX_PAUSED]):
                    row[IDX_TOTAL_TO_DATE] = _to_number(row[IDX_ACTIVE], 0) + _to_number(row[IDX_PAUSED], 0)
                ...
                row[IDX_SOURCE] = "Smartrr product rows: new + active/paused totals"
                row[IDX_DATE_BASIS] = "Smartrr created date; Shopify only fallback if Smartrr unavailable"

            # Do not infer active/paused totals from new subscribers. If Smartrr did not
            # return matching active/paused totals, keep them blank so the dashboard
            # does not display false totals.
            merged.append(row)
        except Exception:
            merged.append(r)
        ...
        try:
            ...
            if active_key not in used_active:
                ...
                if _is_blank_number(row[IDX_PAUSED]):
                    row[IDX_PAUSED] = 0
                if _is_blank_number(row[IDX_TOTAL_TO_DATE]):
                    row[IDX_TOTAL_TO_DATE] = _to_number(row[IDX_ACTIVE], 0) + _to_number(row[IDX_PAUSED], 0)
                merged.append(row)
        except Exception:
            merged.append(r)
    ...
    if active_rows:
        ...
        print(f"    smartrr_product_volume merge: active/paused rows unavailable; using {len(order_rows)} period-new fallback rows with blank totals")
    return merged
...
def write_smartrr_product_volume(gc, sheet_id, rows, periods_to_replace):
    ...
    for r in rows or []:
        ...
        while len(row) < len(SMARTRR_PRODUCT_VOLUME_HEADERS):
            row.append("")
        # Do not invent active_subscribers_to_date or paused_subscribers_to_date from new_subscribers.
        # Blank means unknown/unavailable, not zero.
