...
def build_smartrr_product_volume_from_orders(now_str, brand_name, period, start, end, orders):
    ...
    IMPORTANT separation:
      ...
      - new_subscribers = only subscription line items (selling_plan/subscription/membership).
      - normal one-off products stay in the sales-volume table and DO NOT inflate new subscribers.
    """
    if brand_name != "cavali":
        return []
    ...
    for (product, sku), v in sorted(buckets.items(), key=lambda kv: (-kv[1]["qty"], kv[0][0].lower())):
        ...
        rows.append([
            ...
            product, sku,
            qty, sub_qty,
            sub_qty if is_sub else 0, 0, 0,
            round(v["gross"], 2),
            "Shopify subscription line_items fallback for Smartrr NEW subscribers" if is_sub else "Shopify order line_items fallback for Sales Volume only",
            "Shopify order processed date fallback",
            "row_type=subscription_new" if is_sub else "row_type=sales_volume_only",
            "; ".join(v["ids"]),
        ])
